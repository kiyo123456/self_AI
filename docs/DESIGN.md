...
| 用語 | 定義 |
|------|------|
| 経験（Experience） | 日々の活動・日記・その中で感じたこと・気づいたことが蓄積されたデータのこと。量を蓄積することで価値が生まれる。【制約】ユーザー本人のみ入力できる／削除できる／内容を変更できない／記録日時は入力時に自動付与・変更不可 |
| 推論価値観（InferredValue） | ユーザーの具体的な活動や取り組み・成果物・主観的な考察をもとにシステムによって推論・抽出される抽象的な概念。【制約】ユーザー本人は入力・変更できない |
| 推論スキル（InferredSkill） | 経験・価値観のトリプルを SPARQL + Gemini API で分析した結果として導出されるスキル。ユーザーは直接入力しない |
| 強み（Strength） | 複数の経験から AI によって推論された複数の InferredSkill を材料に、さらに AI が推論した結果として導き出されるもの。思考や行動の無意識的な部分から始まる、自分では当たり前だと思っている部分であり、スキルの土台となる概念。【制約】ユーザーは入力・変更できない |
| トリプル（Triple） | `<主語, 述語, 目的語>` の形式で表現され、システムが作成する知識の最小単位。【制約】ユーザーは作成・変更できない／Experience が登録されたタイミングでシステムが自動生成する |
| 推論セッション（InferenceSession） | ユーザーが「推論を実行」し、InferredSkill / Strength が生成されるまでの一連の操作。【制約】1ユーザーが複数持てる／推論結果は上書きされず新しい Session として蓄積される。【状態】情報スキャン中→分析中→完了／失敗 |
| 分析クエリ（AnalysisQuery） | 推論結果に対して「私の強みは？」などの問いを投げる操作。過去に走らせた InferenceSession の結果をもとに回答する。【制約】ユーザーが作成する／新たな InferenceSession は起動しない |
